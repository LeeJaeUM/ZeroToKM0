...
public class Dealer : NetworkBehaviour
{
    ...
    // 배열을 입력받아 섞어주는 함수
    public void ArrayShuffle(object[] obj)                           // Fisher-Yates 셔플 알고리즘
    {
    ...
    // 리스트를 입력받아 섞어주는 함수
    public void ListShuffle(List<Card> card)                           // Fisher-Yates 셔플 알고리즘
    {
        System.Random random = new System.Random();     // 난수 생성기
        ...
        {
            int j = random.Next(i + 1);                 // 0 ~ i 범위에서 랜덤 인덱스
            Card temp = card[i];                     // 현재 인덱스와 랜덤 인덱스를 스왑
            card[i] = card[j];
            card[j] = temp;
        }
    }
